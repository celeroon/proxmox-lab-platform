#!/usr/bin/env bash
# Build a Cisco Secure Firewall Threat Defense Virtual (FTDv) Proxmox template from Cisco's
...
# a ready-to-import qcow2. The result is UNREGISTERED — registering it to FMC is a post-deploy
# step. lab template build imports the result as a single-use template.
#
# Usage:   build-cisco-ftd.sh <image.qcow2>
# Example: build-cisco-ftd.sh ~/Downloads/Cisco_Secure_Firewall_Threat_Defense_Virtual-10.0.0-140.qcow2
#
# Needs: git, packer >= 1.7, qemu-img, /dev/kvm access, 4 vCPU + 8 GB free.
set -euo pipefail
...
HCL_FILE="cisco-ftd-no-vagrant.pkr.hcl"

WORK_DIR="${WORK_DIR:-/var/lib/lab-platform/build-work/cisco-ftd}"
OUT_DIR="${OUT_DIR:-$PROJECT_ROOT/images/base}"
IMAGE_NAME="ftdv"                       # HCL image_name default (the staged source)
VERSION_LABEL="${VERSION_LABEL:-lab}"   # only names the packer output file; template name is set on import

[[ -f "$SRC" ]] || { echo "error: source not found: $SRC" >&2; exit 1; }

echo "=== Cisco FTDv image builder ==="
echo "  source : $SRC"
echo "  repo   : $REPO_URL"
echo "  output : $OUT_DIR/cisco-ftd.qcow2"
echo
...
# Step 2: verify the source is really a qcow2, then stage it (plus ONLY the FTD hcl — the repo
# also ships the FMC hcl, and packer would choke on duplicate variable blocks) in a clean work
...
echo "[3/4] running packer build (boot -> wizard -> shutdown; this takes ~15-45 min)..."
...
# Packer writes the configured disk to out_dir/vm_name (vm_name = cisco-ftd-<version>.qcow2).
BUILT="$WORK_DIR/tmp_out/cisco-ftd-${VERSION_LABEL}.qcow2"
...
mkdir -p "$OUT_DIR"
mv -f "$BUILT" "$OUT_DIR/cisco-ftd.qcow2"
rm -rf "$WORK_DIR/tmp_out"

echo
echo "Done: $OUT_DIR/cisco-ftd.qcow2"
