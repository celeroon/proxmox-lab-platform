...
def correlate(es, tests):
    ...
    for h in hits:
        ...
        # "What triggered the rule": many rules match non-process events (PowerShell
        # script blocks, registry, network), which have no command line — fall back to
        # the script block, then the raw event message. evt = the event kind, for a note.
        ps_block = ((s.get("powershell") or {}).get("file") or {}).get("script_block_text")
        cats = (s.get("event") or {}).get("category")
        evt = ((s.get("event") or {}).get("action")
               or (cats[0] if isinstance(cats, list) and cats else cats) or "")
        alerts.append(dict(
            rule=s.get("kibana.alert.rule.name"), sev=s.get("kibana.alert.severity"),
            risk=s.get("kibana.alert.risk_score"), techs=techs,
            proc_name=proc.get("name"),
            cmd=(proc.get("command_line") or ps_block or s.get("message") or ""), evt=evt,
...
# ---------- outputs ----------
def build_summary(run, runid, tactics=None):
    s = {"run_id": runid, "run_host": run["host"]}
    if tactics:
        s["tactics"] = tactics
    s.update({
...
def build_html(run, runid, tz, tactics=None):
    e = html.escape
    def cmd(s, n=2000): s = s or ""; return e(s if len(s) <= n else s[:n] + " …")
    tactics_html = (f'tactics <b>{e(", ".join(tactics))}</b> &nbsp;·&nbsp; ' if tactics else '')
    ...
    for t in run["tests"]:
        ...
        exitc = e(str(t["exitc"]))
        exit_html = f'exit {exitc}' if exitc in ("0", "") else f'exit <span class="warn">{exitc}</span>'
        if det:
            rlist = "".join(f'<div class="rline">{r}</div>'
                            for r in sorted({e(a["rule"] or "-") for a in t["alerts"]}))
        else:
            rlist = '<div class="rline sub">no rule matched</div>'
        sumrows.append(f"""<div class="srow {'ok' if det else 'gap'}">
          <div class="sh"><b>{e(t['tech'])} #{e(t['num'])}</b> <span class="sub">{e(t['name'])}</span></div>
          <div class="smeta">{badge} &nbsp;·&nbsp; {tz.t(t['start'])} &nbsp;·&nbsp; {exit_html}</div>
          <div class="rlist">{rlist}</div></div>""")
    ...
    for t in run["tests"]:
        ...
        if t["detected"]:
            ...
            for a in t["alerts"]:
                ...
                if a['cmd']:
                    trig = ('<div class="kv"><b>What triggered the rule</b></div>'
                            f'<div class="cmdblock">{cmd(a["cmd"])}</div>')
                else:
                    trig = ('<div class="kv"><b>What triggered the rule</b> '
                            f'<span class="sub">no command line — matched on '
                            f'{e(a.get("evt") or "a non-process event")}</span></div>')
                proc_html = (f'<div class="kv"><b>process</b> <span class="mono">{e(a["proc_name"])}</span></div>'
                             if a['proc_name'] else '')
                body.append(f"""<div class="al">
                  <div class="rulelabel">Rule</div>
                  <div class="rulename">{e(a['rule'] or '-')}</div>
                  {f'<div class="rulelink">{link}</div>' if link else ''}
                  {trig}
                  {proc_html}
                  {anc}</div>""")
 ...
 * {{ box-sizing: border-box; }}
 ...
 .hdr {{ background:#2d1b4e; color:#fff; padding:11px 12px; }}
 ...
 .legend {{ font-size:10.5px; color:#3b4a66; background:#f7f8fb; border:1px solid #e3e6ee;
            border-radius:6px; padding:7px 10px; margin:0 0 14px; line-height:1.5; }}
 .srows {{ margin:0 0 4px; }}
 .srow {{ border:1px solid #e3e6ee; border-left-width:3px; border-radius:8px; padding:8px 11px; margin-bottom:8px; }}
 .srow.ok {{ border-left-color:{GREEN}; }}
 .srow.gap {{ border-left-color:{AMBER}; background:#fff8f7; }}
 .sh {{ font-size:12.5px; }}
 .smeta {{ font-size:10.5px; color:#55607a; margin:3px 0 5px; }}
 .rline {{ font-size:11.5px; color:#1b2330; padding:1px 0 1px 12px; overflow-wrap:anywhere; word-break:break-word; }}
 ...
 .al {{ margin-top:11px; padding-top:9px; border-top:1px dashed #e7eaf1; }}
 .rulelabel {{ font-size:10px; text-transform:uppercase; letter-spacing:.03em; color:#55607a; font-weight:600; margin-bottom:2px; }}
 .rulename {{ font-size:12.5px; font-weight:600; color:{GREEN}; margin:0 0 3px; overflow-wrap:anywhere; word-break:break-word; }}
 .rulelink {{ margin:0 0 6px; }}
 .rulelink a {{ font-size:11px; color:#5b3fa6; }}
...
<div class="hdr">
  ...
  <div class="meta">run <b>{e(runid)}</b> &nbsp;·&nbsp; host <b>{e(run['host'])}</b> &nbsp;·&nbsp;
    {tactics_html}window {e(win)} {e(tz.label)} &nbsp;·&nbsp; generated {e(gen)} {e(tz.label)}</div>
</div>
<div class="wrap">
  <div class="cards">
  ...
  <div class="srows">{''.join(sumrows)}</div>
  <div class="legend"><b>Reading the outcome:</b> exit <b>0</b> + not detected = a <b>detection gap</b>
    (the test ran, nothing caught it). Exit <b>non-zero</b> + not detected = the atomic <b>failed to run</b>
    (e.g. it needs an interactive session) — not a detection gap.</div>
...
def gotenberg_pdf(html_str, gotenberg_url):
    ...
    # Chromium (Gotenberg) ignores CSS @page margins by default and applies its own
    # ~0.39in border — set tight left/right margins explicitly, keep backgrounds.
    fields = {"marginTop": "0.3", "marginBottom": "0.3",
              "marginLeft": "0.15", "marginRight": "0.15", "printBackground": "true"}
    body = ("".join(part(k, v) for k, v in fields.items())
            + part("files", html_str, filename="index.html", ctype="text/html")
            + f'--{boundary}--\r\n').encode()
...
def main():
    ...
    ap.add_argument("--tactic", default="", help="tactic(s) run this detonation (comma list); shown in the report only when set")
    ...
    summary = build_summary(run, runid, tactics)
    nav = build_navigator(run, runid, args.attack_version)
    doc = build_html(run, runid, tz, tactics)
