"""Live htop-style status view for enip-server, run directly on the VM.

Tails /var/log/enip-server.log — the same file the systemd service already
writes — and renders the sensor+solenoid I/O rack's state as a live-updating
terminal dashboard. Read-only: opens no EtherNet/IP connection of its own, so
it can't be mistaken for a second HMI and never appears in a Malcolm/Zeek
capture.

Run:
    /opt/enip/venv/bin/python3 /opt/enip/enip_server_tui.py
"""
...
def render(reading: Optional[Reading], history: Deque[float], last_update: Optional[datetime]) -> Panel:
    ...
    if reading is None:
        table.add_row("[dim]waiting for enip-server log activity...[/dim]")
    else:
        ...
        table.add_row("SENSOR1", sensor_text)
        table.add_row("SOLENOID1", solenoid_text)
    ...
    if stale:
        table.add_row("", "[bold white on red] NO UPDATES — enip-server may have crashed [/bold white on red]")

    footer = f"last update: {last_update.strftime('%H:%M:%S') if last_update else '-'}  |  Ctrl+C to exit"
    border = "red" if stale else "white"
    return Panel(table, title="enip-server — sensor+solenoid I/O rack simulator", subtitle=footer, border_style=border)


def main() -> None:
    parser = argparse.ArgumentParser(description="Live TUI for enip-server")
    parser.add_argument("--log-file", default="/var/log/enip-server.log")
