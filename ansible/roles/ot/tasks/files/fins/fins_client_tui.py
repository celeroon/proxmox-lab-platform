"""Live htop-style status view for fins-client, run directly on the VM.

Tails /var/log/fins-client.log — the same file the systemd service already
writes — and renders the HMI's view of the conveyor state as a live-updating
terminal dashboard. Read-only: parses the existing log stream, opens no FINS
connection of its own beyond what fins-client is already doing.

Run:
    /opt/fins/venv/bin/python3 /opt/fins/fins_client_tui.py
"""
...
def render(reading: Optional[Reading], counts: Deque[int], last_update: Optional[datetime]) -> Panel:
    ...
    if reading is None:
        table.add_row("[dim]waiting for fins-client log activity...[/dim]")
    else:
        table.add_row("ITEM COUNT (as seen by HMI)", f"[bold cyan]{reading.item_count}[/bold cyan]")
    ...
    if stale:
        table.add_row("", "[bold white on red] NO UPDATES — fins-client may have crashed/disconnected [/bold white on red]")

    footer = f"last poll: {last_update.strftime('%H:%M:%S') if last_update else '-'}  |  Ctrl+C to exit"
    border = "red" if stale else "white"
    return Panel(table, title="fins-client — HMI simulator", subtitle=footer, border_style=border)


def main() -> None:
    parser = argparse.ArgumentParser(description="Live TUI for fins-client")
    parser.add_argument("--log-file", default="/var/log/fins-client.log")
