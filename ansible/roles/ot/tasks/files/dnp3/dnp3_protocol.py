...
layers, just enough to carry: READ (class poll), SELECT/OPERATE/DIRECT_OPERATE
(CROB breaker control), and COLD_RESTART, plus RESPONSE carrying breaker status
+ line current.

Scapy has no DNP3 layer (only Modbus is in scapy.contrib) so this is built
directly with struct, not as scapy Packet subclasses. Deliberate simplifications
from the full DNP3 spec, since this is a lab simulator, not a certified stack:

- No link-layer confirm/ACK/NACK handshake (RESET_LINK_STATES etc.) — every
  frame uses UNCONFIRMED_USER_DATA. Fine over TCP, which is already reliable.
- READ requests are always answered with the full current point set
  (status + current), regardless of which class was actually requested — no
  separate event/class queue. A real outstation differentiates Class 0/1/2/3;
  this one doesn't need to for the lab's purposes.
- Only ever one CROB point (the breaker) and one analog point (the current).

All multi-byte fields are little-endian, matching the real DNP3 spec.
"""
...
def wrap_frame(control: int, dest: int, src: int, user_data: bytes) -> bytes:
    ...
    # CRC covers the sync bytes too (8 bytes total), NOT just the 6-byte
    # header_body -- confirmed empirically 2026-07-05 against Zeek's own DNP3
    # analyzer (zeek -C -r against a clean loopback capture): a 6-byte-scope
    # CRC gets flagged as `dnp3_corrupt_header_checksum` on every single frame,
    # 100% of the time, even though it's a mathematically correct CRC-16/DNP
    # computation (verified against the official check value AND residue for
    # the 6-byte scope) -- Zeek simply expects a wider input span than most
    # informal DNP3 descriptions state.
...
# Responses always carry exactly these two objects, in this fixed order:
# binary_output_status (6 bytes) then analog_input (10 bytes). Encapsulated
# here so callers never have to track the offsets by hand — that's exactly
# the bug this caught during testing (decode_analog_input() called on offset
# 0 instead of offset 6).
