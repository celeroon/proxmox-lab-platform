...
-- Single-use appliance claims: FTD/FMC templates (tagged 'single-use' in Proxmox) are pets
-- registered to FMC/Cisco SSM, so a template may back only ONE live deployment at a time.
-- template is the PRIMARY KEY, so a second deployment naming the same template can't claim it.
-- Claimed at deploy for every single-use template the scenario names (even a disabled node);
-- released when the deployment is destroyed. A claim held by a destroyed deployment is stale
-- and treated as free (see lab/deploy.py).
